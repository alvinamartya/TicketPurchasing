...
namespace TicketPurchasing.MenuAgency
{
    public partial class UclSeat : UserControl
    {
        ...
        public UclSeat(string scheduleID, string cabintype, int left, int mid, int right, int totalSeat, 
            int start, decimal price, UclPurchase purchase, UclRefund refund, string ticketID)//changed
        {
            InitializeComponent();
            lblTitle.Text = cabintype.ToUpper();
            if (purchase != null) //changed
            ...
            int mid, int right, int totalSeat, int start, decimal price)
        {
            ...
            //added
            DataSet refundPassanger = database.getDataFromDatabase("sp_view_transaction_customer_seat",
                new List<Parameter> { new Parameter("@ticketID", this.ticketID) });
            var refundSeatList = refundPassanger.Tables[0].AsEnumerable().Select(
                dataRow => new
                {
                    seatNumber = dataRow.Field<int>("seat"),
                    cabin = dataRow.Field<string>("cabin")
                }).ToList();
            ...
            {   
                ...
                //added
                if (refundSeatList.Where(x => x.cabin == cabintype && x.seatNumber == seatnum).FirstOrDefault() != null)
                                    buttonSeat.Condition = 1;   //blom gua ganti biru
                else if(convertDataSetToList
        ...
        private void btn_click(object sender, EventArgs e)
        {
            ...
            {
                if (btn.Clicked == false && refund.buttonSeat == null && btn.Condition==1)
                {
                    refund.buttonSeat = btn;
                    btn.Clicked = true;
                    //btn.Condition = ; 
                    //gua blom implement, biar ga ngubah ButtonSeat.cs
                }
                else if (btn.Clicked == true && refund.buttonSeat != null)
                {
                    refund.buttonSeat = null;
                    btn.Clicked = false;
                    //btn.Condition = 0;
                }
            }
